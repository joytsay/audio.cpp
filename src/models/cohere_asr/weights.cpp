...
std::unique_ptr<CohereWeights> load_cohere_weights(
    ...
    for (int64_t i = 0; i < 48; ++i) {
        ...
        w.self_attention.pos_weight = store.load_tensor(source, p + ".self_attn.linear_pos.weight", type, {1280, 1280});
        w.self_attention.pos_bias_u = store.load_f32_tensor(source, p + ".self_attn.pos_bias_u", {8, 160});
        w.self_attention.pos_bias_v = store.load_f32_tensor(source, p + ".self_attn.pos_bias_v", {8, 160});
        w.conv.norm = norm(p + ".norm_conv");
        w.conv.pointwise_in = {store.load_tensor_as_shape(source, p + ".conv.pointwise_conv1.weight", type,
            {2560, 1280, 1}, core::TensorShape::from_dims({2560, 1280})),
            store.load_f32_tensor(source, p + ".conv.pointwise_conv1.bias", {2560})};
        w.conv.pointwise_out = {store.load_tensor_as_shape(source, p + ".conv.pointwise_conv2.weight", type,
            {1280, 1280, 1}, core::TensorShape::from_dims({1280, 1280})),
            store.load_f32_tensor(source, p + ".conv.pointwise_conv2.bias", {1280})};
        w.conv.depthwise = binding::depthwise_conv1d_from_source(store, source,
            p + ".conv.depthwise_conv", assets::TensorStorageType::F32, 1280, 9, true);
        w.conv.depthwise_norm = binding::batch_norm_eval_from_source(store, source, p + ".conv.batch_norm", 1280, 1e-5f);
    ...
    out->embedding = store.load_tensor(source, "transf_decoder._embedding.token_embedding.weight", type, {16384, 1024});
