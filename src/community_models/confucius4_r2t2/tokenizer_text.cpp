...
std::shared_ptr<const R2T2ASRTextTokenizer::Impl> load_impl(const R2T2ASRAssets & assets) {
    ...
    auto impl = std::make_shared<R2T2ASRTextTokenizer::Impl>();
...
R2T2ASRTextTokenizer::R2T2ASRTextTokenizer(std::shared_ptr<const R2T2ASRAssets> assets)
    : assets_(std::move(assets)) {
    if (assets_ == nullptr) {
        throw std::runtime_error("R2T2 ASR tokenizer requires assets");
    }
    impl_ = load_impl(*assets_);
}

R2T2ASRPrompt R2T2ASRTextTokenizer::build_prompt(
    const std::string & context,
    const std::string & language,
    const int64_t audio_feature_tokens) const {
    return build_raw_audio_prompt(build_prompt_text(context, language), audio_feature_tokens);
}

std::string R2T2ASRTextTokenizer::build_prompt_text(
    const std::string & context,
...
std::vector<int32_t> R2T2ASRTextTokenizer::encode(const std::string & text) const {
    return impl_->tokenizer->encode(text);
}

R2T2ASRPrompt R2T2ASRTextTokenizer::build_raw_audio_prompt(
    const std::string & text,
    const int64_t audio_feature_tokens) const {
    if (audio_feature_tokens <= 0) {
        throw std::runtime_error("R2T2 ASR prompt requires positive audio feature token count");
    }
...
std::string R2T2ASRTextTokenizer::decode(const std::vector<int32_t> & token_ids) const {
