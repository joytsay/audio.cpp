...
template <typename T>
static void concat_cuda_typed(ggml_tensor * dst, cudaStream_t stream, int32_t dim) {
    ...
    if (ggml_is_contiguous(src0) && ggml_is_contiguous(src1)) {
        ...
        auto launch_kernel = [&](auto dim_value) {
            concat_non_cont<T, decltype(dim_value)::value>
                <<<grid_dim, CUDA_CONCAT_BLOCK_SIZE, 0, stream>>>(
...
void ggml_cuda_op_concat(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    switch (dst->type) {
        case GGML_TYPE_F32:
            concat_cuda_typed<float>(dst, stream, dim);
            break;
        case GGML_TYPE_F16:
            concat_cuda_typed<half>(dst, stream, dim);
            break;
        case GGML_TYPE_BF16:
            concat_cuda_typed<nv_bfloat16>(dst, stream, dim);
            break;
        default:
